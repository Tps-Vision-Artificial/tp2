...
def write_hu_moments(writer):
    ...
    for mom in hu_moments:
        ...
        row = numpy.append(objLabel, flattened)  # le metes el flattened array y le agregas el name
        counter = counter + 1
...
def hu_moments_of_file2(image):
    # Calculate Moments
    moments = cv2.moments(image)  # momentos de inercia
    # Calculate Hu Moments
    huMoments = cv2.HuMoments(moments)  # momentos de Hu
    # Log scale hu moments
    for i in range(0, 7):
